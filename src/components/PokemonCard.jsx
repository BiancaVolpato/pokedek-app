...
import { buscarResumoWiki } from '../services/wikiService';
...
const PokemonCard = ({ pokemon }) => {
    ...
    const [descricaoWiki, setDescricaoWiki] = useState("");
    ...
    useEffect(() => {
        async function carregarDescricao() {
            if (pokemonData.name) {
                const descricao = await buscarResumoWiki(pokemonData.name);
                setDescricaoWiki(descricao);
            }
        }
        carregarDescricao();
    }, [pokemonData.name]);
                ...
                <div className="card-details">
                    ...
                    <div className="t-details">
                        {isLoadingData ? (
                            ...
                            <p>
                                {pokemonData.types?.[1] ? (
                                    <>
                                        <span className='p-d-type-details'>{firstTypeFixed}</span>{" "}
                                        <span className='p-d-type-details'>{secondTypeFixed}</span>
                                    </>
                                ) : (
                                    <span className='p-d-type-details'>{firstTypeFixed}</span>
                                )}
                    ...
                    <div className="f-details">
                        {isLoadingData ? (
                            ...
                            <>
                                <div><p><b>HP</b>: {pokemonData.stats?.[0].base_stat}</p></div>
                                <div><p><b>Ataque</b>: {pokemonData.stats?.[1].base_stat}</p></div>
                                <div><p><b>Defesa</b>: {pokemonData.stats?.[2].base_stat}</p></div>
                            </>
                        )}
                    </div>
                    ...
                    <div className="wiki-description">
                        {descricaoWiki && (
                            <>
                                <h4>Curiosidade da Wikipédia:</h4>
                                <p>{descricaoWiki}</p>
                            </>
                        )}
                    </div>
